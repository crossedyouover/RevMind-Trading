...
function showResearch(report){
  ...
  const grid=node("div","","research-grid");for(const row of report.rows){const card=node("article","","research-card "+row.action.toLowerCase());const top=node("div","","research-card-head");const title=node("div","");title.append(node("strong",row.symbol),node("small",row.exchange+" · "+row.timeframe.replaceAll("_"," ")));top.append(title,node("span",row.action,"decision-pill"));card.append(top,sparkline(row.chart));
    const facts=node("div","","research-facts");for(const [label,value] of [["Latest close",row.latest_close??"—"],["Trend",row.trend??row.trend_status.replaceAll("_"," ")],["Setup",row.active_setups.length?row.active_setups.join(", ").replaceAll("_"," "):"None active"],["Bars analyzed",String(row.bar_count)]]){const fact=node("div","");fact.append(node("small",label),node("strong",value));facts.append(fact);}card.append(facts,node("p",row.explanation,"research-explanation"));if(row.latest_bar_at)card.append(node("small","Latest completed bar: "+row.latest_bar_at.replace("T"," "),"research-time"));grid.append(card);}results.append(grid);
}
...
$("run-market").onclick=async()=>{$("run-market").disabled=true;$("research-status").textContent="ANALYZING";notice("Fetching completed Alpaca bars and running deterministic watchlist research…");try{const report=await api("/api/alpaca/research","POST");showResearch(report);notice("Watchlist research complete. REVIEW is an evidence flag, not an instruction to trade; inspect the setup and apply your own risk decision.");}catch(e){$("research-status").textContent="FAILED SAFELY";notice(e.message);}finally{$("run-market").disabled=false;}};
...
async function loadSettings(){try{const s=await api("/api/settings");$("data-mode").value=s.data_mode;$("feed").value=s.alpaca_feed;$("timeframe").value=s.timeframe;$("session-rule").value=s.session_rule;$("watchlist").value=watchlistText(s.watchlist);$("credentials-state").textContent=s.credentials_configured?"Credentials stored locally":"Credentials not configured";$("provider-status").textContent=s.integration_status.replaceAll("_"," ");$("source-badge").textContent=s.data_mode==="ALPACA"?"ALPACA SELECTED · ON-DEMAND ONLY":"OFFLINE · SYNTHETIC DATA";const ready=s.data_mode==="ALPACA"&&s.credentials_configured;$("test-alpaca").disabled=!ready;$("run-market").disabled=!ready;}catch(e){notice(e.message);}}
