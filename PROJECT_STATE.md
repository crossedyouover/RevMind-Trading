...
- Frozen through: Phase 80 (compact historical performance review)
- Frozen commit: `c9011f166b02f4e149e5b0496c708fa60737777c`
- Frozen tag: `phase80-frozen` (peeled tag resolves to the frozen commit)
...
git rev-parse "phase80-frozen^{}"
git merge-base --is-ancestor "phase80-frozen^{}" HEAD
...
`HEAD` and `origin/main` must match, the peeled Phase 80 tag must resolve to
`c9011f166b02f4e149e5b0496c708fa60737777c`, and the ancestry check must exit successfully. The
...
Phase 80 is frozen on `phase80-frozen`. RevMind can discover bounded account choices and materialize
...
visible while verbose daily observation rows sit behind a local disclosure. Phase 81 will explicitly
invalidate all displayed account and performance results after saved configuration changes or clear,
preventing facts from the prior account context remaining visually current. See `PHASE71_DESIGN.md`
through `PHASE81_DESIGN.md`.
